...
fn assert_fixture(input_file: &str, target_file: &str) {
    ...
    let (stdout, stderr) = simfmt(&[input.as_str(), "-e", "stdout"]);
    ...
    assert_eq!(
        stdout, expected,
        "formatted output differs for {input_file}\nstderr:\n{stderr}"
    );
}
...
ui_tests! {
    ...
    match_arm_blocks: "source/real_contracts/match_arm_blocks.simf" => "target/real_contracts/match_arm_blocks.simf",
